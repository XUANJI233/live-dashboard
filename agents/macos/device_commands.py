"""Device command parsing and desktop-safe execution for macOS."""
...
def extract_device_command(message: object) -> dict | None:
    """Return a device_command envelope from a WS frame or queued message payload."""
...
def execute_desktop_command(envelope: dict) -> tuple[dict, dict | None]:
    """Apply the macOS desktop subset and return (result_frame, synthetic_message)."""
    ...
    result = {
        ...
        "executed_at": now_iso(),
        "actions": actions,
        "state_after": {"desktop_message_visible": bool(actions)},
        "reason": reason,
    }
