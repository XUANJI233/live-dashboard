  ...
  BasePage({
    ...
    build() {
      ...
      this.lastSyncDataWidget = createWidget(widget.TEXT, {
        ...
        align_h: align.CENTER_H,
        text_style: text_style.WRAP,
        text: `${this.state.lastSyncData || '未同步'}`,
      })
    ...
    updateDisplay() {
      ...
      if (this.lastSyncDataWidget) {
        this.lastSyncDataWidget.setProperty(prop.TEXT, this.state.lastSyncData || '未同步')
      }
    },
      ...
      return this.request({
        ...
        .then((result) => {
          ...
          if (result?.skipped) {
            ...
            this.state.lastSyncData = `⏸️ 跳过 (${result.reason || 'unknown'})`
          } else {
        ...
        .catch((error) => {
          ...
          this.state.lastSyncData = `错误: ${String(error).substring(0, 30)}`
          this.updateDisplay()
