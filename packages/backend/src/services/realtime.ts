...
const deleteConversationByViewer = db.prepare(`
  DELETE FROM visitor_messages
  WHERE viewer_id = ? AND device_id = ?
`);
...
export async function handleDeleteMessage(req: Request): Promise<Response> {
  ...
  // Support both single message delete and bulk conversation delete
  const messageId = typeof body.message_id === "string" ? body.message_id : "";
  const viewerId = typeof body.viewer_id === "string" ? body.viewer_id : "";

  if (viewerId) {
    // Delete all messages from this viewer (conversation delete)
    const result = deleteConversationByViewer.run(viewerId, device.device_id);
  ...
  return Response.json({ error: "message_id or viewer_id required" }, { status: 400 });
}
