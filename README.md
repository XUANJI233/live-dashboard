...
阿里云 ESA 默认读取的缓存标签头是 `Cache-Tag`；源站和边缘函数会同时输出 `Cache-Tag` 与 `ESA-Cache-Tag`，因此默认标签刷新和自定义标签头两种配置都能命中。当前状态、今天的时间线、当天健康数据、当前公开留言窗口、位置轨迹和 WebSocket 不缓存；历史时间线、历史健康数据、历史公开留言、配置、健康检查和每日总结会带标签，便于在 CDN 控制台按标签刷新。
